...
void ReadMem(uint8_t Addr, uint8_t* pData, uint16_t Len);
...
void ReadByte(uint8_t Addr, uint8_t* Data)
{
    HAL_I2C_Mem_Read(&OLED_USE_IIC, OLED_ADDR, Addr, OLED_ADDR_SIZE, Data, 1, OLED_IIC_TIMEOUT);
}
...
void ReadMem(uint8_t Addr, uint8_t* pData, uint16_t Len)
{
    HAL_I2C_Mem_Read(&OLED_USE_IIC, OLED_ADDR, Addr, OLED_ADDR_SIZE, pData, Len, OLED_IIC_TIMEOUT);
}
...
void SSD1306_UpdateScreen()
{
    for(uint8_t i = 0; i < 64/8; i++)
    {
        ...
        WriteMem(0x40,&SSD1306_Buffer[128*i],128);
    }
}
